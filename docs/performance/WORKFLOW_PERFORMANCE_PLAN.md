...
- Lot 4 (TOC prep): rapport de contrainte active (`report-constraints`) + seuils dédiés (`CONSTRAINT_TARGETS`/`CONSTRAINT_TREND_TARGETS`) + backlog priorisé (`constraint-actions`) + trend temporel (`constraint-history`/`constraint-trend`) + plan de lot exécutable (`constraint-lot-plan`/`constraint-lot-update`/`constraint-lot-advance`) + boucle CI `plan -> advance -> summary` avec résumés Markdown (`constraint-summary`, `constraint-trend-summary`, `constraint-lot-plan-summary`) et vérifications fixtures dédiées.
