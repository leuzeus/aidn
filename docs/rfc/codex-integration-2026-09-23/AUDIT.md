...
| Validation | `scaffold/codex/drift-check/SKILL.md`; `tools/perf/gating-evaluate.mjs` | Contrôles AIDN et CI existants, dépendants du chemin emprunté | **Compléter** liaison preuve-contenu ; ne pas accepter un booléen de modèle |
| Clôture | `src/application/runtime/workflow-transition-lib.mjs:279-345`; `scaffold/codex/close-session/SKILL.md:20-106` | Refus de clôture sans session/décisions/cycles résolus ; checkpoint existant | **Conserver** moteur ; preuve d'origine humaine des décisions à renforcer |
