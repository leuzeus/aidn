...
6. Disable/unapprove the hook through native UI and record the native observation. The nominal local-metadata diagnostic cannot observe native trust changes: approval must stay unknown and operational status unverified; a future evidence-aware diagnostic must report a witnessed disable as degraded. Check two hook sources, a single multi-alias matcher, concurrent denials/rewrites and repeated delivery. No duplicate effect is acceptable.
