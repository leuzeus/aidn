import { WORKFLOW_ADMISSION_STATUS } from "./workflow-transition-constants.mjs";
import { buildWorkflowRoute, buildWorkflowStatus } from "./workflow-transition-lib.mjs";
...
export function evaluateHandoffAdmission({
  packet,
  current,
  runtime,
  ...
  sharedPlanningFreshness = "not_applicable",
  sharedPlanningGateStatus = "not_applicable",
  sharedPlanningGateReason = "none",
  packetSource = "file",
  packetResolutionInfo = null,
  workspace,
  ...
  for (const field of ["mode", "branch_kind", "active_session", "active_cycle", "dor_state", "first_plan_step"]) {
  ...
  for (const field of ["runtime_state_mode", "repair_layer_status", "current_state_freshness"]) {
    const packetValue = normalizeScalar(packet.get(field) ?? "");
  ...
  const packetUpdatedAtMs = Date.parse(String(packet.get("updated_at") ?? ""));
  const currentUpdatedAtMs = Date.parse(String(current.get("updated_at") ?? ""));
  if (!Number.isNaN(packetUpdatedAtMs) && !Number.isNaN(currentUpdatedAtMs) && packetUpdatedAtMs < currentUpdatedAtMs) {
  ...
  const route = buildWorkflowRoute({
    ...
    reason: `handoff admission ${admissionStatus}`,
  ...
  return {
    packet_file: handoffPacket ? (normalizeScalar(packetRef) || "shared-coordination://handoff_relays") : packetFile,
    ...
    runtime_state_source: runtimeStateSource,
