...
const CLI_EFFECT_POLICIES = freezeDeep([
  ...
  commandPolicy({
    id: "project-config-write",
    command: "aidn project config --init-defaults --write --json",
    effectClass: "mutating",
    jsonContract: "project-config-write.v1.schema.json",
    safeArgs: ["project", "config", "--init-defaults", "--project-name", "write-project", "--write", "--json"],
    notes: "Writes project adapter configuration only after explicit --write intent.",
  }),
  commandPolicy({
  ...
  commandPolicy({
    id: "runtime-repair-layer",
    command: "aidn runtime repair-layer --json",
    effectClass: "preview",
    stability: "internal",
    safeArgs: ["runtime", "repair-layer", "--json"],
    notes: "Builds repair findings; durable application requires --apply.",
  }),
  commandPolicy({
    id: "runtime-repair-layer-query",
    command: "aidn runtime repair-layer-query --json",
    effectClass: "read-only",
    stability: "internal",
    safeArgs: ["runtime", "repair-layer-query", "--query", "baseline-context", "--json"],
    notes: "Queries repair-layer state without applying decisions.",
  }),
  commandPolicy({
    id: "runtime-repair-layer-resolve",
    command: "aidn runtime repair-layer-resolve --json",
    effectClass: "preview",
    stability: "internal",
    safeArgs: ["runtime", "repair-layer-resolve", "--session-id", "S000", "--cycle-id", "C000", "--decision", "accepted", "--json"],
    notes: "Previews a repair decision; durable application requires --apply.",
  }),
  commandPolicy({
    id: "runtime-repair-layer-triage",
    command: "aidn runtime repair-layer-triage --json",
    effectClass: "read-only",
    stability: "internal",
    safeArgs: ["runtime", "repair-layer-triage", "--json"],
    notes: "Reads and summarizes repair findings.",
  }),
  commandPolicy({
    id: "runtime-repair-layer-autofix",
    command: "aidn runtime repair-layer-autofix --json",
    effectClass: "preview",
    stability: "internal",
    safeArgs: ["runtime", "repair-layer-autofix", "--json"],
    notes: "Previews conservative autofix decisions; durable application requires --apply.",
  }),
  commandPolicy({
