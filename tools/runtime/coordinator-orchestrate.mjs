...
export async function orchestrateCoordinatorDispatch(options = {}) {
  ...
  for (let iteration = 0; iteration < args.maxIterations; iteration += 1) {
    const execution = await resumeCoordinatorDispatch(buildCoordinatorResumeOptions(args, true));
    runs.push(execution);
    ...
    if (iteration === args.maxIterations - 1) {
      lastPreview = await resumeCoordinatorDispatch(buildCoordinatorResumeOptions(args, false));
      break;
    }

    const nextPreview = await resumeCoordinatorDispatch(buildCoordinatorResumeOptions(args, false));
    ...
    if (sameCoordinatorDispatch(currentPreview, nextPreview)) {
