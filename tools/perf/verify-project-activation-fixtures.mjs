...
try {
  ...
  const outcomes = await Promise.all(plans.map((plan) => new Promise((resolve, reject) => { const child = spawn(process.execPath, [worker, Buffer.from(JSON.stringify(plan)).toString("base64")], { env: cleanEnv, windowsHide: true, stdio: ["ignore", "pipe", "pipe"] }); let output = ""; child.stdout.on("data", (data) => { output += data; }); child.stderr.on("data", (data) => { output += data; }); child.on("error", reject); child.on("exit", (code) => resolve({ code, output: output.trim() })); })));
  check("two real processes cannot commit conflicting authority plans", () => { assert.equal(outcomes.filter((result) => result.code === 0).length, 1, JSON.stringify(outcomes)); assert(outcomes.some((result) => /AUTHORIZATION_(CAS_CONFLICT|LOCKED)/.test(result.output))); assert.equal(readActivation({ targetRoot: concurrent }).authorization.revision, 1); });
