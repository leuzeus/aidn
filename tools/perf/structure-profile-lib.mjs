...
function recommendedArtifactsForKind(kind) {
  ...
  if (kind === "legacy" || kind === "mixed" || kind === "unknown") {
...
export function detectStructureProfile(auditRoot) {
  ...
  const recommendedRequiredArtifacts = recommendedArtifactsForKind(detected.kind);
  const optionalTrackedArtifacts = [
    "SPEC.md",
    "WORKFLOW_SUMMARY.md",
    "CONTINUITY_GATE.md",
  ...
  return {
    ...
    recommended_required_artifacts: recommendedRequiredArtifacts,
    optional_tracked_artifacts: optionalTrackedArtifacts,
    signals,
    notes,
  };
}
