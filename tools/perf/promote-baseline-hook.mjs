...
import { runPromoteBaselineAdmitUseCase } from "../../src/application/runtime/promote-baseline-admit-use-case.mjs";
...
function printUsage() {
  console.log("Usage:");
  console.log("  node tools/perf/promote-baseline-hook.mjs --target . --mode COMMITTING --json");
  console.log("  node tools/perf/promote-baseline-hook.mjs --target . --mode COMMITTING --index-store dual --json");
}
...
function main() {
  try {
    ...
    const admission = runPromoteBaselineAdmitUseCase({
      targetRoot,
      mode: args.mode,
    });
