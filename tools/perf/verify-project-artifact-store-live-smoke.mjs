...
if (!connectionString) { console.log('UNAVAILABLE: AIDN_RUNTIME_PG_SMOKE_URL is required'); process.exitCode = 1; }
  ...
  try {
    ...
    await upsert({ path: 'cycles/C001-test/status.md', content: closureStatus('VERIFYING', 'VERIFIED') });
    ...
    const closureDrift = runScript('tools/codex/run-json-hook.mjs', ['--skill', 'drift-check', '--mode', 'COMMITTING', '--strict']);
    assert.equal(JSON.parse(closureDrift.stdout).ok, true, closureDrift.stderr);
    await upsert({ path: 'cycles/C001-test/status.md', content: closureStatus('DONE', 'VERIFIED') });
    await upsert({ path: 'CURRENT-STATE.md', content: 'mode: COMMITTING\nactive_session: S001\nactive_cycle: none\n' });
    ...
    await upsert({ path: 'cycles/C001-test/status.md', content: closureStatus('DONE', 'PARTIAL') });
    ...
    console.log('PASS live PostgreSQL: closed-cycle checkpoint, canonical usage-matrix and duplicate-status refusals, ordinary mapping gate, rows and second scope preserved');
