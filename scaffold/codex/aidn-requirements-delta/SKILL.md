---
name: aidn-requirements-delta
description: "AIDN project workflow. Revise an old plan/spec safely by generating an addendum (delta), mapping old→new REQs, and opening CR or new cycle if structural changes are detected. Use only in an explicitly activated AIDN project."
metadata:
  aidn-skill-id: requirements-delta
---
...
Before loading workflow context or running the steps below, run `aidn runtime pre-write-admit --target . --skill requirements-delta --json`. Continue only when `activation.active` is `true` and `admission_status` is `admitted` or `admitted_with_warnings`.
...
- Recommend aidn-drift-check
- If on a mixed branch: recommend aidn-convert-to-spike or new cycle + new branch.
...
- the runtime `aidn-requirements-delta` hook applies ownership/impact admission before delegating to generic checkpoint/index/repair behavior
