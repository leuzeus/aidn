---
name: aidn-branch-cycle-audit
description: "AIDN project workflow. Verify branch ownership for session/cycle/intermediate branches and enforce cycle mapping invariants. Use only in an explicitly activated AIDN project."
metadata:
  aidn-skill-id: branch-cycle-audit
---
...
Before loading workflow context or running the steps below, run `aidn runtime pre-write-admit --target . --skill branch-cycle-audit --json`. Continue only when `activation.active` is `true` and `admission_status` is `admitted` or `admitted_with_warnings`.
...
- This skill begins with a blocking branch-ownership admission decision before generic gating/perf evaluation runs.
- If runtime admission returns `result=stop`, STOP and request the required mapping decision before continuing.

## Steps

1) Run branch-cycle admission first:
- detect current branch
- classify branch kind with the shared workflow mapping layer used by `aidn-start-session`
- resolve the owning session/cycle artifact
- stop on:
  - non-owned branches (`source` / `other` / `unknown`)
  - missing mapping
  - ambiguous mapping
- only continue to generic gating/perf evaluation when branch ownership is valid

2) Identify current Git branch (if accessible).
If not accessible, ask user to provide it.

3) Scan active cycles:
...
5) Check mapping by branch kind:
- Cycle branch:
...
6) If current work mode is COMMITTING:
...
7) Update snapshot (optional but recommended):
...
- the runtime `aidn-branch-cycle-audit` hook applies branch-ownership admission before delegating to generic gating evaluation
