---
name: aidn-close-session
description: "AIDN project workflow. Finalize session, enforce drift check, update snapshot, validate branch context consistency. Use only in an explicitly activated AIDN project."
metadata:
  aidn-skill-id: close-session
---
...
Before loading workflow context or running the steps below, run `aidn runtime pre-write-admit --target . --skill close-session --json`. Continue only when `activation.active` is `true` and `admission_status` is `admitted` or `admitted_with_warnings`.
...
- the runtime `aidn-close-session` hook applies open-cycle resolution admission before delegating to generic `session-close` checkpoint/index/repair behavior
...
If cycle DONE, recommend using aidn-promote-baseline workflow.
If the session is review-ready, recommend running `aidn-pr-orchestrate` immediately after `aidn-close-session`.
