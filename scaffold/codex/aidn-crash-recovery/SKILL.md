---
name: aidn-crash-recovery
description: "AIDN project workflow. Recover safely after an abrupt stop, restart, or suspected partial runtime write in an aidn repository using shared coordination. Use when local backlog or handoff artifacts may be missing or stale, when PostgreSQL shared state may be ahead of local files, or when the next safe action is to reanchor, rematerialize planning, regenerate a handoff packet, or restore shared coordination. Use only in an explicitly activated AIDN project."
metadata:
  aidn-skill-id: crash-recovery
---
...
Before loading workflow context or running the steps below, run `aidn runtime pre-write-admit --target . --skill crash-recovery --json`. Continue only when `activation.active` is `true` and `admission_status` is `admitted` or `admitted_with_warnings`.
