...
description: "AIDN project workflow. Create or resume a session file from template, detect branch context, and prepare cycle-aware working context. Use only in an explicitly activated AIDN project."
metadata:
  aidn-skill-id: start-session
---
...
Before loading workflow context or running the steps below, run `aidn runtime pre-write-admit --target . --skill start-session --json`. Continue only when `activation.active` is `true` and `admission_status` is `admitted` or `admitted_with_warnings`.
...
1) Run aidn-start-session runtime admission first:
...
2) Run aidn-context-reload logic (light version):
